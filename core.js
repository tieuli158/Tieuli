// ====================================================================================
// ||                      HỆ THỐNG QUẢN LÝ TRẠNG THÁI (ĐÃ SỬA LỖI)                  ||
// ====================================================================================

// Hàm này sẽ tự động chạy ngay khi file core.js được tải để kiểm tra quyền truy cập
(function() {
    console.log("Đang kiểm tra trạng thái hệ thống...");

    // 1. Kiểm tra xem biến SYSTEM_STATUS từ file status.js có tồn tại không
    if (typeof SYSTEM_STATUS === 'undefined') {
        // Trường hợp 1: Không tải được file status.js (do mạng hoặc link sai)
        // Mặc định cho phép chạy để tránh tool bị chết do lỗi mạng
        console.log("⚠️ Không đọc được file trạng thái, chạy chế độ mặc định.");
    } else {
        // Trường hợp 2: Đã tải được file status.js -> Kiểm tra giá trị
        console.log("✅ Trạng thái hệ thống: " + SYSTEM_STATUS);
        
        if (SYSTEM_STATUS === 0) {
            // Nếu là 0 -> Kích hoạt bảo trì ngay lập tức
            const msg = (typeof MSG_BAOTRI !== 'undefined') ? MSG_BAOTRI : "Hệ thống đang bảo trì nâng cấp.";
            activeMaintenanceMode(msg);
            
            // QUAN TRỌNG: Ném lỗi để dừng toàn bộ code phía sau không cho chạy
            throw new Error("⛔ Tool stopped by Administrator."); 
        }
    }
})();

// Hàm hiển thị màn hình khóa (Giao diện đen ngầu, không thể tắt)
function activeMaintenanceMode(message) {
    document.body.innerHTML = ''; // Xóa sạch giao diện cũ
    ...
    lockScreen.style.cssText = `
        ...
        z-index: 999999; padding: 20px; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;
    `;
    
    lockScreen.innerHTML = `
        <div style="font-size: 5rem; margin-bottom: 20px; animation: pulse 2s infinite;">🛡️</div>
        <h1 style="color: #ff4757; font-size: 2.5rem; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 2px;">BẢO TRÌ HỆ THỐNG</h1>
        <div style="background: rgba(255,255,255,0.1); padding: 20px; border-radius: 10px; border: 1px solid #333; max-width: 600px;">
            <p style="font-size: 1.2rem; line-height: 1.6; color: #ced6e0; margin: 0;">
                ${message.replace(/\n/g, '<br>')}
            </p>
        </div>
        <div style="margin-top: 40px; font-size: 0.9rem; color: #555;">Administrator Access Control</div>
...
// Ẩn màn hình loading (Nếu tool được phép chạy)
...
function md5(string) {
    ...
    function convertToWordArray(string) {
        var lWordCount; var lMessageLength = string.length;
