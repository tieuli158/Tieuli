...
// KIỂM TRA TRẠNG THÁI HOẠT ĐỘNG NGAY KHI LOAD
if (TOOL_CONFIG.STATUS !== "ACTIVE") {
    document.body.innerHTML = `
        <div style="display:flex; flex-direction:column; justify-content:center; align-items:center; height:100vh; background:#0f0c29; color:white; font-family:sans-serif; text-align:center;">
            <h1 style="color:#ff4757; font-size:40px;">⛔ THÔNG BÁO TỪ SERVER</h1>
            <p style="font-size:18px; margin:20px 0;">${TOOL_CONFIG.MESSAGE}</p>
            <p style="color:#2ed573;">${TOOL_CONFIG.CONTACT}</p>
            <button onclick="location.reload()" style="margin-top:20px; padding:10px 20px; cursor:pointer;">Thử lại</button>
        </div>
    `;
    throw new Error("Tool has been disabled by Administrator.");
}

// --- PHẦN LOGIC CHÍNH (KHÔNG ĐƯỢC XÓA) ---
